...
// Falling snow, entirely in the vertex shader: every flake drifts on its own
// phase and wraps around a box that follows the camera, so the field is endless.
export function createSnowfall(scene, cfg) {
	...
	const material = new THREE.ShaderMaterial({
		...
		uniforms,
		vertexShader: /* glsl */`
			attribute vec4 aSeed;   // size, fall speed, sway radius, phase
			...
			void main(){
				...
				// Wrap the horizontal field around the camera.
		...
		fragmentShader: /* glsl */`
			...
			void main(){
				...
				// Six faint arms so larger flakes read as crystals, not dots.
