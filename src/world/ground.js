...
const TERRAIN = /* glsl */`
	// Rolling snow that flattens into the clearing where the board rests.
...
// Snow: displaced ground with wind-carved drifts, blue shadow scatter and a
// glitter layer that only fires when the camera catches a facet.
	...
	material.onBeforeCompile = shader => {
		...
		shader.fragmentShader = shader.fragmentShader
			...
			.replace('#include <color_fragment>', `#include <color_fragment>
				// Wind streaks run north-east; crust reads slightly warmer than powder.
			...
			.replace('#include <opaque_fragment>', `
				{
					// Facet glitter: one candidate flake per cell, lit only when the
					// view vector lines up with its randomised tilt.
