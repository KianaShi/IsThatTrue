...
// One conifer, merged into a single geometry so a whole stand is one draw call.
	...
	material.onBeforeCompile = shader => {
		...
		shader.fragmentShader = shader.fragmentShader
			...
			.replace('#include <color_fragment>', `#include <color_fragment>
				// Needles darken with depth into the canopy; snow settles on what faces up.
...
export function createForest(scene, cfg) {
	...
	while (buckets.flat().length < count && guard++ < count * 12) {
		...
		// Keep a sightline open to the north-west so the sun reaches the board.
		...
		buckets[v].push({
			...
			lean: 0.82 + random() * 0.42,   // some stands run tall and thin, others squat
