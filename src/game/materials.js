...
const RECIPES = {
	ivory: {
		...
		glsl: {
			grain: `
				// Bone grain: long striations up the turned axis, plus fine pores.
			...
			light: `
				// Cheap translucency: light bleeding around the silhouette.
	...
	obsidian: {
		...
		glsl: {
			grain: `
				// Conchoidal fracture: broad shells with a cold sheen inside them.
...
// Every piece owns a material so it can glow on its own; they share one
// compiled program because the injected source is identical per recipe.
	...
	material.onBeforeCompile = shader => {
		...
		shader.fragmentShader = shader.fragmentShader
			...
			.replace('#include <color_fragment>', `#include <color_fragment>
				${recipe.glsl.grain}
				// Snow settles on the shoulders of every piece.
