...
// Lathes and boxes arrive indexed, extrusions do not; flatten before merging.
const merge = parts => {
...
// Small turner's language for lathe profiles: straight runs, quadratic
// fillets and circular arcs, all in board units (one square = 1.0).
function turn() {
	const pts = [];
	const api = {
		pts,
...
// Foot common to every piece: a weighted disc, a chamfer and a collar.
function foot(p, r) {
...
// The knight is the one piece a lathe cannot make: a pedestal plus a carved
// side profile, tapered across its thickness so it reads as a head from every
// angle rather than a plank. It faces -z, down the board.
function knight() {
	const p = foot(turn(), 0.252);
	...
	// Narrow towards the muzzle and the ears; the cheeks stay full.
